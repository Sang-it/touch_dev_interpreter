...
describe("TYPESCRIPT: While Statement", () => {
	test(`Check that statementParser can parse a while statement with body.`, async () => {
		...
		const actualCode = await prettierFormat(
			whileStatementInstance.createCodeBlock()
		);

		const expectedCode = await prettierFormat(
			"while (true) {let x: number = 10;let y: number = 20;console.log(x, y);}"
		);
